...
public class QueryPlan {
    ...
    public QueryOperator minCostSingleAccess(String table) {
        ...
        int indexWillBeUsed = -1;
        // Get all indexed columns estimate I/O cost.
        for (int indexedColumnIndex : getEligibleIndexColumns(table)) {
            SelectPredicate predicate = selectPredicates.get(indexedColumnIndex);
            QueryOperator thisOp = new IndexScanOperator(
                    this.transaction,
                    predicate.tableName,
                    predicate.column,
                    predicate.operator,
            ...
            int thisOpCost = thisOp.estimateIOCost();

            // Compare indexOp and get this min cost.
            if (Math.min(minCost, thisOpCost) != minCost) {
                minCost = thisOpCost;
                minOp = thisOp;
                indexWillBeUsed = indexedColumnIndex;
            }
        }

        // If we use `indexScan` method, remove selection predicate on `indexWillBeUsed`.
        // The output of `indexScan` method is same as the output of `WHERE index` selection.
        minOp = addEligibleSelections(minOp, indexWillBeUsed);
